...
let Server = (() => {
	function getModules(PubSub, EventMashine, ConnectionLogger) {
		let monitor = Monitor.create(this.redisClient, PubSub, ConnectionLogger),
			session = Session.create(this.dbhPool, this.redisClient, EventMashine, ConnectionLogger, monitor);
		return {
			'Journal': Journal.create(this.dbhPool, this.redisClient, session, EventMashine, ConnectionLogger),
			'Session': session,
			'Monitor': monitor,
			'PubSub': PubSub,
