...
let Session = (() => {
	...
	var dbhPool, redisClient, eventMashine, logger, monitor;
...
SET
	...
	class Session
	{
		constructor(/* dbhPool, redisClient, eventMashine, logger, monitor */) {
			dbhPool = arguments[0];
...
LIMIT ${page * SESSIONS_PAGE_LIMIT - SESSIONS_PAGE_LIMIT}, ${SESSIONS_PAGE_LIMIT}
					...
					(err, rows) => {
						...
						response.list = rows.map(session => {
							try {
								session.additional = JSON.parse(session.additional);
								return session;
							} catch(e) {
								reject(e);
							}
		...
		getSessionsByUuidList(uuidList) { return new Promise((resolve, reject) => {
			if(!(typeof(uuidList) === "object" && uuidList.length > 0)) {
				return reject(new LogRoomReject(LogRoomStatus.INCORRECT_UUID_LIST));
			}
			dbhPool.getConnection((err, conn) => {
				if(err) {
					conn.release();
				...
				let quotedList = uuidList.map(uuid => conn.escape(uuid));
				conn.query(`
SELECT
	uuid,
	data as additional
FROM session
WHERE uuid IN(${quotedList.join(',')})
				`, (err, rows) => {
					conn.release();
					if(err) {
						return reject(err);
					...
					return resolve(new Response(LogRoomStatus.SUCCESS, rows));
				});
			});
		}); }

		async getActiveSessions() {
			let uuidList = await monitor.getActiveUuidList();

			return uuidList.length > 0
				? Promise.resolve(await this.getSessionsByUuidList(uuidList))
				: Promise.resolve(new Response(LogRoomStatus.SUCCESS, []));
		}
	}
...
exports.create = (dbhPool, redisClient, eventMashine, logger, monitor) => {
	return new Session(dbhPool, redisClient, eventMashine, logger, monitor);
};